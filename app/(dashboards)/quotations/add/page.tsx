...
const isItemFilled = (item: any) => {
  ...
  const blankItem = () => ({
    ...
    price: 0, discount: 0, discount_type: "percentage", qty: 1, total: 0,
  });
...
const removeRow = (index: number) => {
  ...
  const updateItem = (index: number, key: string, value: any) => {
    ...
    const discountAmount = discountType === "percentage" ? (price * qty * discount) / 100 : discount;
  ...
  const getFinalTotal = () => {
    let total = subtotal;
    if (form.discount_type === "percentage") total -= (total * Number(form.discount)) / 100;
    else total -= Number(form.discount);
    if (form.tax_type === "percentage") total += (total * Number(form.tax)) / 100;
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-2 py-3 md:px-6 md:py-6 overflow-hidden">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-3 md:p-5 mb-3 shadow-sm">
        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Quotation Details</h3>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">

          <div>
            <label className={labelCls}>Quotation Date</label>
            <input type="date" value={form.quotation_date} readOnly className={inputCls} />
          </div>
            ...
            return (
              <div key={index} className="grid grid-cols-[1fr_1fr_80px_150px_60px_80px_72px] gap-2 items-center mb-2">
                ...
                <div className="flex gap-1 items-center">
                  {isEditing ? (
                    <button onClick={() => setEditingIndex(null)} className="p-1.5 bg-green-100 text-green-600 rounded-lg">
                      <Check size={13} />
                    </button>
                  ) : (
                  ...
                  <button onClick={() => removeRow(index)} className="p-1.5 bg-red-100 text-red-600 rounded-lg">
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
              ...
              <div key={index} className="border border-gray-200 rounded-xl p-3 bg-gray-50">
                ...
                {isEditing ? (
                  ...
                  <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
                    ...
                    <div><span className="text-gray-400">Product: </span><span className="font-medium">{item.item_name || "—"}</span></div>
                    ...
                    <div><span className="text-gray-400">Total: </span><span className="font-semibold text-gray-900">{item.total.toFixed(2)}</span></div>
                  </div>
                )}
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-3 md:p-5 shadow-sm ml-auto w-full md:max-w-xs space-y-2.5">
        ...
        <button onClick={handleSubmit} disabled={loading}
          className="bg-gray-900 text-white w-full py-2.5 rounded-xl text-sm hover:bg-gray-700 transition disabled:opacity-60 font-medium">
          {loading ? "Saving..." : "Save Quotation"}
        </button>
      </div>
