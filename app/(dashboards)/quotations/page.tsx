...
//   const itemName = editItems[index]?.item_name || "Item";
...
export default function QuotationsPage() {
  ...
  const [activityQuotation, setActivityQuotation] = useState<any>(null);
  ...
  const formatDiscount = (quotation: any) => {
    const discount = Number(quotation?.discount || 0);
    const subtotal = Number(quotation?.subtotal || 0);

    if (quotation?.discount_type === "percentage" && subtotal > 0) {
  ...
  const getItemChanges = (oldItems: any[] = [], newItems: any[] = []) => {
    ...
    newItems.forEach((newItem) => {
      ...
      if (Number(oldItem.quantity) !== Number(newItem.quantity)) {
        changes.push({
          type: "UPDATED",
          text: `${getItemName(newItem)} quantity changed ${
            oldItem.quantity
          } → ${newItem.quantity}`,
        });
      }
      ...
      if (Number(oldItem.unit_price) !== Number(newItem.unit_price)) {
        changes.push({
          type: "UPDATED",
          text: `${getItemName(newItem)} price changed ${
            oldItem.unit_price
          } → ${newItem.unit_price}`,
        });
      }
      ...
      if (
        ...
        changes.push({
          type: "UPDATED",
          text: `${getItemName(newItem)} discount type changed ${
            oldItem.discount_type || "percentage"
          } → ${newItem.discount_type || "percentage"}`,
        });
      }

      if (Number(oldItem.discount_percent) !== Number(newItem.discount_percent)) {
        changes.push({
          type: "UPDATED",
          text: `${getItemName(newItem)} discount changed ${
            oldItem.discount_percent
      ...
      if (Number(oldItem.total) !== Number(newItem.total)) {
        changes.push({
          type: "UPDATED",
          text: `${getItemName(newItem)} total changed ${oldItem.total} → ${
            newItem.total
          }`,
        });
      }
  ...
  const normalizeAmountType = (type: any, defaultType = "percentage") => {
  ...
  const openActivity = async (quotation: any) => {
    if (!quotation?._id) return;

    setActivityQuotation(quotation);
    setActivityLogs([]);
    setActivityLoading(true);

    try {
      const res = await AuditLogService.getEntityAuditLogs(
        "QUOTATION",
        quotation._id
      );
  ...
  const openEdit = async (quotation: any) => {
    ...
    const rawDiscount =
      discountType === "percentage" && Number(quotation?.subtotal || 0) > 0
        ? (
            (Number(quotation.discount || 0) / Number(quotation.subtotal || 0)) *
            100
          ).toFixed(2)
        : quotation?.discount || 0;
    ...
    const rawTax =
      taxType === "percentage" && taxable > 0
        ? ((Number(quotation.tax || 0) / taxable) * 100).toFixed(2)
        : quotation?.tax || 0;
    ...
    try {
      ...
      if (res?.success) {
        ...
        toast.error(res?.message || "Failed to load quotation items");
        setEditQuotation(null);
      }
    } catch {
      toast.error("Failed to load quotation items");
  ...
  const updateEditItem = (index: number, key: string, value: any) => {
    ...
    if (["unit_price", "quantity", "discount_percent"].includes(key)) {
      updated[index][key] = value === "" ? 0 : Number(value) || 0;
    } else {
      updated[index][key] = value;
    }
    ...
    updated[index].discount_type = discountType;
  ...
  const removeEditItem = (index: number) => {
    ...
    if (!isEditItemFilled(item)) {
      ...
      setEditItems(updated.length > 0 ? updated : [emptyEditItem()]);
      return;
    }

    setDeleteEditItemConfirm({
      index,
      item,
    });
  };
  ...
  const handleConfirmEditItemDelete = () => {
    ...
    const index = deleteEditItemConfirm.index;
    const itemName = getItemName(deleteEditItemConfirm.item);
  ...
  const computedTotals = useMemo(() => {
    ...
    const taxAmt =
      editForm.tax_type === "percentage"
        ? (taxable * Number(editForm.tax || 0)) / 100
        : Number(editForm.tax || 0);
    ...
    return {
      ...
      discountAmt,
      taxAmt,
      total: Math.max(taxable + taxAmt, 0),
    };
  }, [editItems, editForm]);
  ...
  const handleUpdate = async () => {
    ...
    const validItems = editItems.filter(isEditItemFilled);
    ...
    if (
      validItems.some(
        (item) => !item.product_id && !String(item.item_name || "").trim()
      )
    ) {
      return toast.error("Please select product or enter item name");
    }
    ...
    try {
      ...
      const payload = {
        ...
        discount_type: normalizeAmountType(editForm.discount_type, "fixed"),
  ...
  const handleConfirm = async () => {
    if (!confirmId || !confirmAction || confirmLoading) return;
  ...
  const getConfirmTitle = () => {
    if (confirmAction === "approve") return "Approve Quotation";
    if (confirmAction === "reject") return "Reject Quotation";
    return "Delete Quotation";
  };

  const getConfirmMessage = () => {
    if (confirmAction === "approve") {
      return "Are you sure you want to approve this quotation? After approval, salesman can download its PDF.";
    }

    if (confirmAction === "reject") {
      return "Are you sure you want to reject this quotation? This action will mark it as rejected.";
    }

    return "Are you sure you want to delete this quotation? This action cannot be undone.";
  };
    ...
    <div className="w-full max-w-7xl mx-auto px-2 py-3 md:px-6 md:py-6 overflow-hidden">
      {confirmId && confirmAction && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl p-5 w-full max-w-sm">
            <div
              className={`flex items-center justify-center w-11 h-11 rounded-full mx-auto mb-3 ${
                confirmAction === "approve" ? "bg-green-50" : "bg-red-50"
              }`}
            >
              {confirmAction === "approve" ? (
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                onClick={handleConfirm}
                disabled={confirmLoading}
                className={`flex-1 py-2 text-sm text-white rounded-xl transition font-medium disabled:opacity-50 flex items-center justify-center gap-2 ${
                  confirmAction === "approve"
              ...
              <>
                  ...
                  <div>
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-md font-medium ${
                        statusStyle[formatStatus(viewQuotation?.status)] || ""
                      }`}
                    >
                      {formatStatus(viewQuotation?.status)}
                    </span>
                ...
                <div className="overflow-y-auto flex-1 p-4 space-y-4">
                  ...
                  <div className="bg-gray-50 rounded-xl p-3 space-y-1.5 text-sm">
                    <div className="flex justify-between">
                      <span className="text-gray-500">Subtotal</span>
                      <span>{formatMoney(viewQuotation?.subtotal)}</span>
                    </div>

                    <div className="flex justify-between">
                      <span className="text-gray-500">Discount</span>
                      <span>- {formatDiscount(viewQuotation)}</span>
                    </div>

                    <div className="flex justify-between">
                      <span className="text-gray-500">Tax</span>
                      <span>+ {formatMoney(viewQuotation?.tax)}</span>
                    </div>

                    <div className="flex justify-between font-semibold border-t pt-2">
                      <span>Total</span>
                      <span>{formatMoney(viewQuotation?.total)}</span>
                    </div>
                  </div>
                </div>
              </>
          ...
          <div className="bg-white w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl shadow-xl max-h-[85vh] flex flex-col">
            <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100">
              <div>
                ...
                <p className="text-xs text-gray-400">
                  {activityQuotation?.quotation_number || "-"}
                </p>
              </div>
              ...
              <>
                <div className="flex items-center justify-between px-4 py-3 border-b border-gray-100 flex-shrink-0">
                  <div>
                    ...
                    <span
                      className={`text-xs px-2 py-0.5 rounded-md font-medium ${
                        statusStyle[formatStatus(editQuotation?.status)] || ""
                      }`}
                    >
                      {formatStatus(editQuotation?.status)}
                    </span>
                ...
                <div className="overflow-y-auto flex-1 p-4 space-y-4">
                                ...
                                <td className="px-1.5 py-1.5">
                                  <select
                                    ...
                                    onChange={async (e) => {
                                      ...
                                      if (categoryId && !productMap[categoryId]) {
                                        const res =
                                          await QuotationService.getProductsByCategory(
                                            categoryId
                                          );
                  ...
                  <div className="bg-gray-50 rounded-xl p-3 space-y-2 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-red-500 text-xs shrink-0">
                        - {formatMoney(computedTotals.discountAmt)}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-green-600 text-xs shrink-0">
                        + {formatMoney(computedTotals.taxAmt)}
                      </span>
                    </div>

                    <div className="flex justify-between font-semibold border-t pt-2">
                      <span>Total</span>
                      <span>{formatMoney(computedTotals.total)}</span>
                    </div>
                  </div>
                </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <div className="relative hidden md:block w-72">
            ...
            <input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Search quotation, dealer, salesman..."
              className="w-full pl-8 pr-3 py-2 text-sm bg-white border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-gray-900"
            />
          </div>
        ...
        <div className="overflow-x-auto">
          <table
            className={`w-full min-w-[850px] text-sm ${
              listLoading && quotations.length > 0 ? "opacity-60" : ""
            }`}
          >
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200 text-xs text-gray-500 uppercase tracking-wider">
              ...
              {listLoading && quotations.length === 0 ? (
                ...
                filtered?.map((quotation: any) => (
                  ...
                  >
                    ...
                    <td className="py-3 px-4 text-gray-600">
                      {formatDiscount(quotation)}
                    </td>

                    <td className="py-3 px-4">
                      <span
                        className={`px-2.5 py-1 text-xs rounded-lg font-medium ${
                          statusStyle[formatStatus(quotation.status)] || ""
                        }`}
                      >
                        {formatStatus(quotation.status)}
                      </span>
                    ...
                    <td className="py-3 px-4 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button
                          type="button"
                          onClick={() => openActivity(quotation)}
                ...
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      type="button"
                      onClick={() => openActivity(quotation)}
                      className="p-1.5 bg-purple-50 hover:bg-purple-100 rounded-lg"
                      title="History"
                    >
