...
import {
  Eye, Plus, Search, SlidersHorizontal, Pencil, X,
  Trash2, MoreVertical, Download, Save, Loader2,
} from "lucide-react";
...
export default function QuotationsPage() {
  ...
  const computedTotals = useMemo(() => {
    ...
    const discountAmt = editForm.discount_type === "percentage"
      ? (subtotal * Number(editForm.discount || 0)) / 100
      : Number(editForm.discount || 0);
    const taxAmt = editForm.tax_type === "percentage"
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-gray-500 mb-6">
              {confirmAction === "approve" ? "Are you sure you want to approve this quotation?"
               : confirmAction === "reject" ? "Are you sure you want to reject this quotation? This cannot be undone."
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => { setConfirmId(null); setConfirmAction(null); }}
                className="px-4 py-2 rounded-lg border text-gray-600 hover:bg-gray-50 text-sm">Cancel</button>
            ...
            {viewLoading ? (
              ...
              <>
                <div className="flex items-center justify-between mb-5">
                  <div>
                    <h2 className="text-lg font-semibold text-gray-800">{viewQuotation?.quotation_number}</h2>
                    <span className={`text-xs px-2 py-1 rounded-md font-medium ${statusStyle[formatStatus(viewQuotation?.status)] || ""}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-5">
                  ...
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <p className="text-xs text-gray-400 mt-2 mb-1">Valid Until</p>
                    <p className="text-sm font-medium">
                      {viewQuotation?.valid_until ? new Date(viewQuotation.valid_until).toLocaleDateString("en-GB") : "-"}
                    </p>
                  </div>
                </div>
                ...
                {/* Items table */}
                <div className="mb-5">
                  <p className="text-sm font-semibold text-gray-700 mb-3">Quotation Items</p>
                      ...
                      <tbody>
                        {viewItems?.length > 0 ? viewItems.map((item: any, i: number) => (
                          <tr key={i} className="border-t">
                            ...
                            <td className="px-4 py-3 text-center">{item?.unit_price || 0}</td>
                            <td className="px-4 py-3 text-center">{item?.discount_percent || 0}</td>
                            <td className="px-4 py-3 text-right">{Number(item?.total || 0).toFixed(2)}</td>
                          </tr>
                        )) : (
                          <tr><td colSpan={5} className="px-4 py-6 text-center text-gray-400">No items found</td></tr>
                        )}
                      </tbody>
                    </table>
                  </div>
            ...
            {editFetchLoading ? (
              ...
              <>
                ...
                <div className="flex items-center justify-between mb-6">
                  <div>
                    <h2 className="text-lg font-semibold text-gray-800">Edit Quotation — {editQuotation?.quotation_number}</h2>
                    <span className={`text-xs px-2 py-1 rounded-md font-medium ${statusStyle[formatStatus(editQuotation?.status)] || ""}`}>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <select
                      ...
                      onChange={(e) => setEditForm({ ...editForm, dealer_id: e.target.value })}
                    ...
                    <p className="text-xs text-gray-400 mt-3 mb-1">Created By</p>
                ...
                <div className="mb-6">
                  ...
                  <textarea rows={2} value={editForm.deliveryNotes || ""}
                ...
                <div className="mb-6">
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-sm font-semibold text-gray-700">Quotation Items</p>
                    <button onClick={addEditItem} className="flex items-center gap-1 text-xs bg-black text-white px-3 py-1 rounded-lg">
                      + Add Item
                    </button>
                  </div>
                  <div className="border rounded-xl overflow-hidden">
                ...
                <div className="bg-gray-50 rounded-xl p-4 space-y-3 text-sm mb-6">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select value={editForm.discount_type || "fixed"}
                    ...
                    <span className="text-red-500 w-20 text-right shrink-0">- {computedTotals.discountAmt.toFixed(2)}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500 w-20 shrink-0">Tax</span>
                    <select value={editForm.tax_type || "fixed"}
                      onChange={(e) => setEditForm({ ...editForm, tax_type: e.target.value })}
                      className="border rounded-lg px-2 py-1 text-xs w-20 bg-white focus:outline-none">
                      <option value="fixed">Amt</option>
                      <option value="percentage">%</option>
                    </select>
                    <input type="number" value={editForm.tax || ""}
                      onChange={(e) => setEditForm({ ...editForm, tax: e.target.value })}
                      className="flex-1 border rounded-lg px-2 py-1 text-xs bg-white min-w-0 focus:outline-none" />
                    <span className="text-green-600 w-20 text-right shrink-0">+ {computedTotals.taxAmt.toFixed(2)}</span>
                  </div>
                ...
                {/* Footer */}
                <div className="flex justify-end gap-3">
                  <button onClick={closeEdit} className="px-4 py-2 rounded-lg border text-gray-600 hover:bg-gray-50 text-sm">Cancel</button>
          ...
          <tbody>
            {filtered?.map((quotation: any) => (
              <tr key={quotation._id} className="border-b last:border-none hover:bg-gray-50">
                ...
                <td>{quotation.created_by?.name} ({quotation.created_by?.user_type === "admin" ? "Director" : "Salesman"})</td>
                ...
                <td>{quotation.valid_until ? new Date(quotation.valid_until).toLocaleDateString("en-GB") : "-"}</td>
      ...
      {openMenu && (
        ...
        >
          {(() => {
            ...
            return (
              <>
                <button onClick={() => { handleView(q._id); setOpenMenu(null); }}
                  className="block w-full text-left px-4 py-2 hover:bg-gray-100 text-sm">View</button>

                {canEdit(q) && (
                  <button onClick={() => { openEdit(q); setOpenMenu(null); }}
                    className="block w-full text-left px-4 py-2 hover:bg-gray-100 text-sm">Edit</button>
                )}

                {user?.user_type === "admin" && (q.status === "pending" || q.status === "rejected") && (
                  <button onClick={() => { setConfirmId(q._id); setConfirmAction("approve"); setOpenMenu(null); }}
                    className="block w-full text-left px-4 py-2 text-green-600 hover:bg-gray-100 text-sm">Approve</button>
                )}

                {user?.user_type === "admin" && (q.status === "pending" || q.status === "approved") && (
                  <button onClick={() => { setConfirmId(q._id); setConfirmAction("reject"); setOpenMenu(null); }}
                    className="block w-full text-left px-4 py-2 text-red-600 hover:bg-gray-100 text-sm">Reject</button>
                )}

                {canDownloadPDF(q) && (
                  <button onClick={() => { handleDownloadPDF(q._id); setOpenMenu(null); }}
                    className="block w-full text-left px-4 py-2 hover:bg-gray-100 text-sm">Download PDF</button>
                )}

                {canDelete(q) && (
                  <button onClick={() => { setConfirmId(q._id); setConfirmAction("delete"); setOpenMenu(null); }}
                    className="block w-full text-left px-4 py-2 text-red-600 hover:bg-red-50 text-sm">Delete</button>
                )}
              </>
            );
          })()}
