...
//   const res = await ProductService.deleteProduct(deleteProductId, user?.industry);
...
export default function ProductsTable({ products = [], refetch }: any) {
  ...
  const openDetails = async (p: any) => {
    ...
    try {
      const res = await ProductService.getProductById(p._id);

      if (res?.success) {
        setDetailProduct(res?.product || p);
      } else {
  ...
  const openActivity = async (p: any) => {
    ...
    try {
      ...
      if (logsRes?.success) {
        setActivityLogs(logsRes?.data || logsRes?.logs || []);
      } else {
        setActivityLogs([]);
      }
    } catch {
  ...
  const handleUpdate = async () => {
    if (!editProduct?._id || loading) return;
  ...
  const handleDelete = async () => {
    if (!deleteProductId || deleteLoading) return;
  ...
  const ProductImage = ({ product, size = "small" }: any) => {
    const className =
      size === "large"
        ? "w-full h-full object-cover"
        : "w-11 h-11 rounded-xl object-cover border border-gray-200 shrink-0";
  ...
  return (
    <>
            ...
            {detailLoading ? (
              ...
              <div className="overflow-y-auto p-5 space-y-5">
                <div className="grid grid-cols-1 sm:grid-cols-[120px_1fr] gap-4">
                  <div className="w-full h-32 sm:w-28 sm:h-28 rounded-2xl overflow-hidden bg-gray-100 border border-gray-200 flex items-center justify-center">
                    {detailProduct?.image ? (
                      <ProductImage product={detailProduct} size="large" />
                    ) : (
                      <Package size={28} className="text-gray-400" />
                    )}
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      ...
                      <div className="bg-gray-50 rounded-xl p-3">
                        <p className="text-xs text-gray-400">Category</p>
                        <p className="font-medium text-gray-800">
                          {detailProduct?.category_id?.name || "-"}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-sm">
                    <div className="bg-gray-50 rounded-xl p-3">
                      <p className="text-xs text-gray-400">MRP</p>
                      <p className="font-semibold text-gray-900">
                        Rs {formatMoney(detailProduct?.mrp)}
                      </p>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3">
                      <p className="text-xs text-gray-400">Discount Amount</p>
                      <p className="font-semibold text-red-500">
                        Rs {formatMoney(detailProduct?.discountAmount)}
                      </p>
                    </div>
                    ...
                    <div className="bg-gray-50 rounded-xl p-3">
                      <p className="text-xs text-gray-400">Sale Price</p>
                      <p className="font-semibold text-green-600">
                        Rs {formatMoney(detailProduct?.salePrice || detailProduct?.mrp)}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                    <div className="bg-gray-50 rounded-xl p-3">
                      <p className="text-xs text-gray-400">Status</p>
                      <span
                        className={`inline-flex mt-1 px-2.5 py-1 rounded-lg text-xs font-medium ${statusColor(
                          Boolean(detailProduct?.is_active)
                        )}`}
                      >
      ...
      {editProduct && isAdmin && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md max-h-[92vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold">Edit Product</h2>
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                ...
                onClick={closeEdit}
                disabled={loading}
                className="px-4 py-2 rounded-lg border text-gray-600 hover:bg-gray-50 text-sm disabled:opacity-50"
              >
                Cancel
              </button>
              ...
              <button
                ...
                onClick={handleUpdate}
                disabled={loading}
                className="px-4 py-2 rounded-lg bg-black text-white text-sm hover:bg-gray-800 disabled:opacity-50 flex items-center gap-2"
              >
      ...
      <div className="mb-3 flex flex-col sm:flex-row gap-2 sm:items-center sm:justify-between">
        ...
        <select
          ...
          className="w-full sm:w-40 bg-white border border-gray-200 rounded-xl px-3 py-2 text-sm outline-none"
        >
            ...
            >
              ...
              <div className="border-t border-gray-100 flex">
                ...
                <button
                  type="button"
                  onClick={() => openActivity(p)}
                  className="flex-1 flex items-center justify-center gap-1.5 py-2.5 text-xs font-medium text-purple-600 hover:bg-purple-50 transition"
                >
                  <History size={13} />
                  History
                </button>
